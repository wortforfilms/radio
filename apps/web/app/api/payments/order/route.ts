...
import { purchaseAmount, type Currency, type ProductType } from "@shared/commerce";
import {
  createRazorpayOrder,
  paymentsConfigured,
  RZP_KEY_ID,
...
export async function POST(request: NextRequest) {
  ...
  // Server-side pricing — never trust a client-supplied amount.
  ...
  const { minor } = purchaseAmount(productType, currency, {
    ...
    album: album
      ? {
          ...
          freeTracks: album.freeTracks,
          priceInrOverride: album.priceInrOverride,
          priceUsdOverride: album.priceUsdOverride,
        }
      : undefined,
  });
  ...
  return Response.json({
    phkd: {
      policy: "fail_closed",
      note: "Order created. No entitlement is granted until a signature-verified webhook confirms capture.",
    },
    keyId: RZP_KEY_ID,
    orderId: order.id,
    amount: minor,

...
import { verifyWebhookSignature, appendPaymentEvidence } from "../../../../lib/payments";
...
export async function POST(request: NextRequest) {
  ...
  if (!verifyWebhookSignature(raw, signature)) {
    return Response.json({ error: "PHKD_FAIL_CLOSED", message: "invalid or missing webhook signature" }, { status: 400 });
  }
  ...
  if ((type === "payment.captured" || type === "order.paid") && payment?.order_id) {
    const purchase = await prisma.purchase.findFirst({ where: { providerOrderId: payment.order_id } });

    if (purchase && purchase.status !== "paid") {
      const receiptId: string = payment.id || purchase.receiptId || `pay_${eventId}`;
      const updated = await prisma.purchase.update({
        where: { id: purchase.id },
        data: { status: "paid", paidAt: new Date(), providerPaymentId: payment.id ?? null, receiptId },
      });
